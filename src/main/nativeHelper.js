...
class NativeHelper {
  ...
  exe() {
    return this.candidates().find(p => fs.existsSync(p)) || '';
  }

  async start() {
    const exe = this.exe();
    if (!exe) {
      const checked = this.candidates().join(' | ');
      throw new Error(`Native helper not found. Checked: ${checked}`);
    }
    ...
    this.proc.stderr.on('data', data => {
      ...
      if (text) this.logger?.warn?.(`native stderr: ${text}`);
    });
    this.proc.on('error', error => {
      this.lastError = error.message;
      this.logger?.error?.(`native process error: ${error.message}`);
    });
  ...
  onData(data) {
    ...
    while (this.buffer.includes('\n')) {
      ...
      try {
        ...
        this.logger?.warn?.(`native invalid json: ${error.message}`);
      }
    }
  }
  ...
  async onExit(code) {
    ...
    if (this.restarts < 1) {
      ...
      try { await this.start(); }
        ...
        this.lastError = error.message;
        this.logger?.error?.(`native restart failed: ${error.message}`);
      }
    }
  }
  ...
  request(command, timeout = 5000) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`native request timeout: ${command.cmd}`));
      }, timeout);
  ...
  async healthCheck() {
    try {
      ...
      return { ok:false, executable:this.exe(), error:error.message, candidates:this.candidates() };
    }
  }
