...
for(const token of ["types:['screen','window']",'screen:set-selection','NEXA_STARTUP_SMOKE_FILE','helperAvailable','rendererReady'])if(!main.includes(token))throw new Error(`main.js contract missing: ${token}`);
const capture=fs.readFileSync(path.join(root,'src/renderer/screenCapture.js'),'utf8');
for(const token of ['MultiSourceCapture','maxSources=16','this.active=new Map()','snapshotBusy','source_count','share_set_id','SELECT APP'])if(!capture.includes(token))throw new Error(`Multi-source capture contract missing: ${token}`);
